...
_aggregator = None
_merged_df = None
_solar_data = None  # dict keyed by building_id


def set_global_state(data_processor, model_results, aggregator, merged_df, solar_data=None):
    global _data_processor, _model_results, _aggregator, _merged_df, _solar_data
    _data_processor = data_processor
    _model_results = model_results
    _aggregator = aggregator
    _merged_df = merged_df
    _solar_data = solar_data
...
# ============================================================
# Solar Potential Endpoints
# ============================================================

@router.get(
    "/buildings/{building_id}/solar",
    response_model=SolarPotential,
    summary="Get solar potential for a building",
    responses={404: {"model": ErrorResponse}}
)
async def get_building_solar(building_id: int):
    if _solar_data is None or not _solar_data:
        raise HTTPException(status_code=503, detail="Solar data not loaded.")
    
    bld = _solar_data.get(building_id)
    if bld is None:
        raise HTTPException(
            status_code=404,
            detail=f"No solar potential data for building {building_id}"
        )
    return SolarPotential(**bld)


@router.get(
    "/solar/summary",
    response_model=SolarSummary,
    summary="Get campus-wide solar potential summary",
)
async def get_solar_summary():
    if _solar_data is None or not _solar_data:
        raise HTTPException(status_code=503, detail="Solar data not loaded.")
    
    buildings = list(_solar_data.values())
    
    total_yearly_kwh = sum(b['solar_yearly_kwh_dc'] for b in buildings)
    total_annual_savings = sum(b['estimated_annual_savings'] for b in buildings)
    total_panel_count = sum(b['solar_max_panel_count'] for b in buildings)
    total_roof_area = sum(b['solar_roof_area_m2'] for b in buildings)
    
    # Average demand offset (only for buildings with actual consumption)
    offsets = [b['solar_demand_offset_pct'] for b in buildings
               if b.get('current_annual_kwh') and b['current_annual_kwh'] > 0]
    ...
    for b in buildings:
        ...
        if tier not in tier_map:
            tier_map[tier] = {'count': 0, 'kwh': 0.0, 'savings': 0.0}
        tier_map[tier]['count'] += 1
        tier_map[tier]['kwh'] += b['solar_yearly_kwh_dc']
        tier_map[tier]['savings'] += b['estimated_annual_savings']
    
    # Order tiers from best to worst
    tier_order = [
        'Top Priority', 'Excellent', 'Very Good', 'Good',
        'Moderate', 'Fair', 'Low Priority'
    ]
    tier_breakdown = []
    for tier_name in tier_order:
        if tier_name in tier_map:
            t = tier_map[tier_name]
            tier_breakdown.append(SolarTierBreakdown(
                tier=tier_name,
                count=t['count'],
                total_yearly_kwh=round(t['kwh'], 0),
                total_annual_savings=round(t['savings'], 0),
            ))
    
    return SolarSummary(
        total_buildings=len(buildings),
        total_yearly_kwh=round(total_yearly_kwh, 0),
        total_annual_savings=round(total_annual_savings, 0),
        total_panel_count=total_panel_count,
        total_roof_area_m2=round(total_roof_area, 1),
        avg_demand_offset_pct=round(avg_offset, 1),
...
async def health_check():
    return {
        ...
        "solar_buildings": len(_solar_data) if _solar_data else 0
    }
