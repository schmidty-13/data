...
# Cost per kWh for solar savings calculation
COST_PER_KWH = 0.10


def load_solar_data(data_dir: str) -> dict:
    """
    Load solar_scores_final.csv and return a dict keyed by building_id.
    Each value is a flat dict ready to be serialized as a SolarPotential response.
    """
    solar_path = Path(data_dir) / "solar_scores_final.csv"
    if not solar_path.exists():
        print(f"  Solar data file not found at {solar_path} -- skipping solar features")
        return {}

    df = pd.read_csv(solar_path)
    print(f"\n  Loading solar data: {len(df)} buildings from {solar_path.name}")

    result = {}
    for _, row in df.iterrows():
        bid = int(row['buildingnumber'])
        yearly_kwh = float(row.get('solar_yearly_kwh_dc', 0) or 0)
        current_kwh = row.get('total_kwh')
        # Handle NaN / inf
        if current_kwh is not None and not (math.isnan(current_kwh) or math.isinf(current_kwh)):
            current_kwh = float(current_kwh)
        else:
            current_kwh = None
        demand_offset = float(row.get('solar_demand_offset_pct', 0) or 0)
        if math.isinf(demand_offset) or math.isnan(demand_offset):
            demand_offset = 0.0

        floors = row.get('floorsaboveground')
        if floors is not None and not (math.isnan(floors) if isinstance(floors, float) else False):
            floors = int(floors)
        else:
            floors = None

        age = row.get('building_age')
        if age is not None and not (math.isnan(age) if isinstance(age, float) else False):
            age = int(age)
        else:
            age = None
        ...
        result[bid] = {
            'building_id': bid,
            'building_name': str(row.get('buildingname', '')),
            'solar_potential_score': round(float(row.get('solar_potential_score', 0)), 2),
            'solar_tier': str(row.get('solar_tier_detailed', row.get('solar_tier', 'Unknown'))),
            'solar_roof_area_m2': round(float(row.get('solar_roof_area_m2', 0)), 1),
            'solar_max_panel_count': int(row.get('solar_max_panel_count', 0)),
            'solar_yearly_kwh_dc': round(yearly_kwh, 0),
            'solar_sunshine_hours_year': round(float(row.get('solar_sunshine_hours_year', 0)), 1),
            'solar_shade_percent': round(float(row.get('solar_shade_percent', 0)), 1),
            'estimated_annual_savings': round(yearly_kwh * COST_PER_KWH, 0),
            'solar_demand_offset_pct': round(demand_offset, 1),
            'current_annual_kwh': current_kwh,
            'radiation_score': round(float(row.get('radiation_score', 0)), 4),
            'rooftop_score': round(float(row.get('rooftop_score', 0)), 4),
            'shade_score': round(float(row.get('shade_score', 0)), 4),
            'demand_score': round(float(row.get('demand_score', 0)), 4),
            'suitability_score': round(float(row.get('suitability_score', 0)), 4),
            'gross_area': float(row.get('grossarea', 0)) if row.get('grossarea') else None,
            'floors_above_ground': floors,
            'building_age': age,
        }

    total_kwh = sum(b['solar_yearly_kwh_dc'] for b in result.values())
    total_savings = sum(b['estimated_annual_savings'] for b in result.values())
    print(f"  Solar data loaded: {len(result)} buildings")
    print(f"  Total solar potential: {total_kwh:,.0f} kWh/yr (${total_savings:,.0f}/yr savings)")
    return result
...
def initialize_data(data_dir: str, use_cache: bool = True):
    ...
    """
    global data_processor, model_results, aggregator, merged_df, solar_data
    
    print("=" * 60)
    print("ECO-PULSE BACKEND INITIALIZATION")
    print("=" * 60)
    
    # Load solar data (lightweight, always runs -- no caching needed)
    solar_data = load_solar_data(data_dir)
    ...
    if use_cache and _cache_is_valid(data_dir):
        ...
        set_global_state(data_processor, model_results, aggregator, merged_df, solar_data)
